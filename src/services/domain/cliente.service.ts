...
export class ClienteService {
  ...
  findByEmail(email: string): Observable<ClienteDTO> {
    return this.http.get<ClienteDTO>(
      `${environment.BASE_URL}/clientes/findbyemail?value=${email}`
    );
  }
  ...
  insert(obj: ClienteDTO) {
    return this.http.post(`${environment.BASE_URL}/clientes/add`, obj, {
      observe: "response",
      responseType: "text"
    });
  }
  ...
  savePlayerId(player: string) {
    let formData: FormData = new FormData();
    formData.set("player", player);
    return this.http.post(
      `${environment.BASE_URL}/clientes/save-playerid`, formData,
      {
        observe: "response",
        responseType: "text"
      }
